...
int main() {
    std::shared_ptr<Shape> floor = create_sphere(Transform::scale(10, 0.01, 10), 1);
    // floor->material = Material();
    floor->material.colour = Colour(1, 0.9, 0.9);
    floor->material.specular = 0;

    Matrix4x4 transform = Transform::translate(0, 0, 5)
                     .mul(Transform::rotate_y(-M_PI / 4))
                     .mul(Transform::rotate_x(M_PI / 2))
                     .mul(Transform::scale(10, 0.01, 10));
    std::shared_ptr<Shape> left_wall = create_sphere(transform, 1);
    ...
    transform = Transform::translate(0, 0, 5)
           ...
           .mul(Transform::rotate_x(M_PI / 2))
           .mul(Transform::scale(10, 0.01, 10));
    std::shared_ptr<Shape> right_wall = create_sphere(transform, 1);
    ...
    std::ofstream fout("world_demo.ppm");
    if (!fout.is_open()) {
